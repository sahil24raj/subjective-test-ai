...
import { X, ShieldCheck, ArrowRight, AlertCircle, Building2, GraduationCap, BookOpen, Lock, Sparkles, CheckCircle2 } from 'lucide-react';
...
export const GoogleAuthModal: React.FC<GoogleAuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const [mode, setMode] = useState<'picker' | 'onboarding'>('picker');
  ...
  const handleFirebaseGoogleAuth = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        setErrorMessage('Firebase API key missing or invalid. Please check .env.local configuration.');
      } else {
        setErrorMessage(err.message || 'Google Auth Popup failed. Please try again.');
      }
    }
  };
    ...
    <div className="fixed inset-0 z-50 bg-[#020512]/85 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
      <div className="w-full max-w-md bg-[#080d26] border border-cyan-500/30 rounded-3xl p-6 sm:p-7 space-y-6 shadow-[0_0_50px_rgba(0,240,255,0.18)] relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col text-left">
              ...
              <span className="font-rajdhani text-xs text-slate-400 font-semibold">
                {mode === 'onboarding' ? 'Academic Profile Setup' : 'Sign in with Google OAuth 2.0'}
              </span>
            </div>
          </div>
          <button
        ...
        {isAuthenticating ? (
          ...
          <div className="space-y-5">
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full py-2.5 rounded-xl border border-slate-800 hover:border-cyber-blue/40 bg-slate-900/60 text-slate-300 hover:text-white font-mono text-[11px] font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-1.5"
              >
          ...
          /* MODE 2: Onboarding Academic Details */
